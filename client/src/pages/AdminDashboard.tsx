...
import { ExternalLink, ImagePlus, Link2, Loader2, Pencil, Plus, Save, ShieldAlert, Sparkles, Trash2, Upload } from "lucide-react";
...
function ProjectsEditor() {
  ...
  const { data: aiStatus } = trpc.cms.aiStatus.useQuery();
  ...
  const generateAI = trpc.cms.generateProjectContent.useMutation({
    onSuccess: (result) => {
      if (result.success && result.data) {
        const d = result.data;
        setForm((current) => ({ ...current, title: d.title || current.title, titleAr: d.titleAr || current.titleAr, category: d.category || current.category, summary: d.summary || current.summary, summaryAr: d.summaryAr || current.summaryAr }));
        play("success");
        toast.success("تم توليد المحتوى بالذكاء الاصطناعي. راجع وحرر قبل الحفظ.");
      }
    },
    onError: (error) => { play("error"); toast.error(error.message); },
  });
  ...
  const saveDraft = () => {
    if (!form.title.trim() || !form.category.trim() || !form.summary.trim() || !form.imageUrl.trim()) return toast.error("أدخل عنوان المشروع ونوعه والوصف والصورة أولًا.");
    const payload = { title: form.title.trim(), titleAr: form.titleAr.trim() || null, category: form.category, summary: form.summary.trim(), summaryAr: form.summaryAr.trim() || null, imageUrl: form.imageUrl.trim(), projectUrl: form.projectUrl.trim() || null, sortOrder: Number(form.sortOrder) || 0, isPublished: false };
    if (form.id) update.mutate({ id: form.id, ...payload }); else create.mutate(payload);
  };

  const publish = () => {
    if (!form.title.trim() || !form.category.trim() || !form.summary.trim() || !form.imageUrl.trim()) return toast.error("أدخل عنوان المشروع ونوعه والوصف والصورة أولًا.");
    const payload = { title: form.title.trim(), titleAr: form.titleAr.trim() || null, category: form.category, summary: form.summary.trim(), summaryAr: form.summaryAr.trim() || null, imageUrl: form.imageUrl.trim(), projectUrl: form.projectUrl.trim() || null, sortOrder: Number(form.sortOrder) || 0, isPublished: true };
    if (form.id) update.mutate({ id: form.id, ...payload }); else create.mutate(payload);
  };
  ...
  return <section className="space-y-6"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[.18em] text-cyan-300">Graphic Project Library</p><h1 className="mt-2 font-serif text-4xl text-white">إدارة المشاريع</h1><p className="mt-2 text-sm text-slate-400">أضف مشاريع جديدة أو حدّثها أو أخفها من صفحة الأعمال العامة.</p></div><button onClick={() => { setForm(EMPTY_PROJECT); setIsFormOpen(true); }} className="inline-flex items-center gap-2 rounded-xl bg-cyan-300 px-4 py-2.5 text-sm font-bold text-[#042027] transition hover:bg-cyan-200"><Plus size={17} />مشروع جديد</button></div>{isFormOpen && <article className="rounded-3xl border border-cyan-200/20 bg-[#08262b] p-5 shadow-2xl"><div className="mb-5 flex items-center justify-between"><h2 className="text-xl font-semibold text-white">{form.id ? "تعديل المشروع" : "إضافة مشروع"}</h2><button onClick={() => setIsFormOpen(false)} className="text-sm text-slate-400 hover:text-white">إلغاء</button></div><div className="grid gap-4 md:grid-cols-2"><Field label="عنوان المشروع — English" value={form.title} onChange={(value) => setForm((current) => ({ ...current, title: value }))} /><Field label="عنوان المشروع — العربية (اختياري)" value={form.titleAr} onChange={(value) => setForm((current) => ({ ...current, titleAr: value }))} /><label className="block text-sm font-medium text-slate-300">التصنيف<select className="mt-2 w-full rounded-xl border border-cyan-100/15 bg-[#071c21] px-3 py-2.5 text-sm text-slate-100 outline-none transition focus:border-cyan-300/60 focus:ring-2 focus:ring-cyan-300/15" value={form.category} onChange={(event) => setForm((current) => ({ ...current, category: event.target.value }))}>{categoryOptions.map((category) => <option key={category} value={category}>{category}</option>)}</select></label><Field label="رابط المشروع (اختياري)" value={form.projectUrl} onChange={(value) => setForm((current) => ({ ...current, projectUrl: value }))} /><Field label="ترتيب الظهور" value={String(form.sortOrder)} onChange={(value) => setForm((current) => ({ ...current, sortOrder: Number(value) || 0 }))} /><div className="md:col-span-2 grid gap-4 md:grid-cols-2"><Field label="وصف المشروع — English" value={form.summary} multiline onChange={(value) => setForm((current) => ({ ...current, summary: value }))} /><Field label="وصف المشروع — العربية (اختياري)" value={form.summaryAr} multiline onChange={(value) => setForm((current) => ({ ...current, summaryAr: value }))} /></div>{aiStatus?.configured && <div className="md:col-span-2 rounded-2xl border border-violet-300/20 bg-violet-500/5 p-4"><label className="block text-sm font-medium text-slate-300">Project brief for AI (English)<textarea dir="auto" className="mt-2 w-full rounded-xl border border-cyan-100/15 bg-[#071c21] px-3 py-2.5 text-sm text-slate-100 outline-none transition focus:border-cyan-300/60 focus:ring-2 focus:ring-cyan-300/15 min-h-20 resize-y" placeholder="Describe the project in English to generate title, category, and summary..." value={aiDescription} onChange={(event) => setAiDescription(event.target.value)} /></label><p className="mt-2 text-xs text-slate-500">Type a description of the project, then click "Generate with AI" below. The AI will auto-fill the title, category, and summary fields.</p></div>}<div className="md:col-span-2 rounded-2xl border border-dashed border-cyan-100/20 bg-black/10 p-4"><div className="flex flex-wrap items-center gap-3"><label className="inline-flex cursor-pointer items-center gap-2 rounded-lg border border-cyan-200/20 px-3 py-2 text-sm text-cyan-100 hover:bg-cyan-100/10"><Upload size={16} />اختر صورة<input className="hidden" type="file" accept="image/*" onChange={(event) => onFile(event.target.files?.[0])} /></label>{upload.isPending && <span className="text-sm text-cyan-200">جارٍ رفع الصورة...</span>}{form.imageUrl && <img src={form.imageUrl} alt="معاينة المشروع" className="h-16 w-24 rounded-lg object-cover" loading="lazy" decoding="async" />}<span className="text-xs text-slate-500">حد أقصى 8 ميغابايت، صيغ PNG أو JPEG أو WebP.</span></div><input className="mt-3 w-full rounded-lg border border-white/10 bg-[#071c21] px-3 py-2 text-sm text-slate-200 outline-none focus:border-cyan-300/60" placeholder="رابط الصورة" value={form.imageUrl} onChange={(event) => setForm((current) => ({ ...current, imageUrl: event.target.value }))} /></div><div className="flex flex-wrap items-center gap-3 mt-2">{aiStatus?.configured ? <button onClick={() => { if (!form.imageUrl) return toast.error("ارفع صورة المشروع أولًا."); generateAI.mutate({ imageUrl: form.imageUrl, language, title: form.title || undefined, category: form.category, summary: form.summary || undefined, adminDescription: aiDescription || undefined }); }} disabled={generateAI.isPending || !form.imageUrl} className="inline-flex items-center gap-2 rounded-xl bg-violet-500/90 px-4 py-2.5 text-sm font-bold text-white transition hover:bg-violet-400 disabled:opacity-50"><Sparkles size={16} />{generateAI.isPending ? "جارٍ تحليل التصميم..." : "Generate with AI"}</button> : <span className="inline-flex items-center gap-2 rounded-xl bg-white/5 px-4 py-2.5 text-sm text-slate-500 cursor-not-allowed"><Sparkles size={16} />AI not configured</span>}</div>{generateAI.isError && <p className="mt-2 text-xs text-rose-400">Generation failed: {generateAI.error.message}</p>}{generateAI.isSuccess && <p className="mt-2 text-xs text-emerald-400">Content generated. Review and edit below, then save or publish.</p>}</div><div className="flex flex-wrap items-center gap-3 mt-4"><button onClick={saveDraft} disabled={create.isPending || update.isPending || upload.isPending} className="inline-flex items-center gap-2 rounded-xl bg-cyan-300/80 px-5 py-3 text-sm font-bold text-[#042027] disabled:opacity-60"><Save size={17} />حفظ كمسودة</button><button onClick={publish} disabled={create.isPending || update.isPending || upload.isPending} className="inline-flex items-center gap-2 rounded-xl bg-cyan-300 px-5 py-3 text-sm font-bold text-[#042027] disabled:opacity-60"><Save size={17} />نشر المشروع</button></div></article>}<div className="grid gap-4 lg:grid-cols-2">{projects?.map((project) => <article key={project.id} className="flex overflow-hidden rounded-2xl border border-white/10 bg-[#08262b]/85"><img src={project.imageUrl} alt="" className="h-36 w-32 object-cover" loading="lazy" decoding="async" /><div className="min-w-0 flex-1 p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-xs uppercase tracking-[.13em] text-cyan-300">{project.category}</p><h2 className="mt-1 truncate text-lg font-semibold text-white">{project.title}</h2></div><span className={`rounded-full px-2 py-1 text-xs ${project.isPublished ? "bg-cyan-300/15 text-cyan-200" : "bg-white/10 text-slate-400"}`}>{project.isPublished ? "منشور" : "مسودة"}</span></div><p className="mt-2 line-clamp-2 text-sm leading-5 text-slate-400">{project.summary}</p><div className="mt-3 flex gap-2"><button onClick={() => { setForm({ id: project.id, title: project.title, titleAr: project.titleAr ?? "", category: project.category, summary: project.summary, summaryAr: project.summaryAr ?? "", imageUrl: project.imageUrl, projectUrl: project.projectUrl ?? "", sortOrder: project.sortOrder, isPublished: project.isPublished }); setAiDescription(""); setIsFormOpen(true); }} className="inline-flex items-center gap-1 text-xs text-cyan-200 hover:text-cyan-100"><Pencil size={14} />تعديل</button><button onClick={() => { if (window.confirm(`حذف ${project.title}?`)) remove.mutate({ id: project.id }); }} className="inline-flex items-center gap-1 text-xs text-rose-300 hover:text-rose-200"><Trash2 size={14} />حذف</button>{project.projectUrl && <a href={project.projectUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-xs text-slate-300 hover:text-white"><ExternalLink size={14} />رابط</a>}</div></div></article>)}</div>{!projects?.length && <div className="rounded-3xl border border-dashed border-white/15 p-10 text-center text-slate-400"><ImagePlus className="mx-auto mb-3 text-cyan-300" /><p>لا توجد مشاريع بعد. أضف مشروعك الأول الآن.</p></div>}</section>;
}
...
function ContactLinksEditor() {
  ...
  return <section className="space-y-6"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[.18em] text-cyan-300">Contact Directory</p><h1 className="mt-2 font-serif text-4xl text-white">روابط وأرقام التواصل</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400">أضف هاتفك وواتساب وروابط حساباتك. ستظهر فقط القنوات المنشورة في صفحة التواصل.</p></div><button onClick={() => { setForm(EMPTY_CONTACT_LINK); setIsFormOpen(true); }} className="inline-flex items-center gap-2 rounded-xl bg-cyan-300 px-4 py-2.5 text-sm font-bold text-[#042027] transition hover:bg-cyan-200"><Plus size={17} />وسيلة جديدة</button></div>{isFormOpen && <article className="rounded-3xl border border-cyan-200/20 bg-[#08262b] p-5 shadow-2xl"><div className="mb-5 flex items-center justify-between"><h2 className="text-xl font-semibold text-white">{form.id ? "تعديل وسيلة تواصل" : "إضافة وسيلة تواصل"}</h2><button onClick={() => setIsFormOpen(false)} className="text-sm text-slate-400 hover:text-white">إلغاء</button></div><div className="grid gap-4 md:grid-cols-2"><Field label="الاسم — English" value={form.label} onChange={(value) => setForm((current) => ({ ...current, label: value }))} /><Field label="الاسم — العربية (اختياري)" value={form.labelAr} onChange={(value) => setForm((current) => ({ ...current, labelAr: value }))} /><label className="block text-sm font-medium text-slate-300">نوع وسيلة التواصل<select className="mt-2 w-full rounded-xl border border-cyan-100/15 bg-[#071c21] px-3 py-2.5 text-sm text-slate-100 outline-none transition focus:border-cyan-300/60 focus:ring-2 focus:ring-cyan-300/15" value={form.type} onChange={(event) => setForm((current) => ({ ...current, type: event.target.value as ContactLinkType }))}>{CONTACT_LINK_TYPES.map((type) => <option key={type} value={type}>{CONTACT_LINK_LABELS[type].ar} / {CONTACT_LINK_LABELS[type].en}</option>)}</select></label><Field label="ترتيب الظهور" value={String(form.sortOrder)} onChange={(value) => setForm((current) => ({ ...current, sortOrder: Number(value) || 0 }))} /><Field label="الرابط أو الرقم" value={form.url} onChange={(value) => setForm((current) => ({ ...current, url: value }))} /></div><label className="mt-3 flex items-center gap-2 text-sm text-slate-300"><input type="checkbox" checked={form.isPublished} onChange={(event) => setForm((current) => ({ ...current, isPublished: event.target.checked }))} />نشر وسيلة التواصل</label><div className="mt-4 flex gap-3"><button onClick={save} disabled={create.isPending || update.isPending} className="inline-flex items-center gap-2 rounded-xl bg-cyan-300 px-5 py-2.5 text-sm font-bold text-[#042027] disabled:opacity-60"><Save size={17} />{form.id ? "تحديث" : "إضافة"}</button><button onClick={() => setIsFormOpen(false)} className="text-sm text-slate-400 hover:text-white">إلغاء</button></div></article>}<div className="space-y-3">{links?.map((link) => <article key={link.id} className="flex items-center gap-4 rounded-2xl border border-white/10 bg-[#08262b]/85 p-4"><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><p className="truncate text-sm font-semibold text-white">{link.label}</p><span className={`rounded-full px-2 py-0.5 text-xs ${link.isPublished ? "bg-cyan-300/15 text-cyan-200" : "bg-white/10 text-slate-400"}`}>{link.isPublished ? "منشور" : "مخفية"}</span></div><p className="mt-1 truncate text-xs text-slate-400">{link.type} — {link.url}</p></div><div className="flex gap-2"><button onClick={() => { setForm({ id: link.id, label: link.label, labelAr: link.labelAr ?? "", type: link.type as ContactLinkType, url: link.url, sortOrder: link.sortOrder, isPublished: link.isPublished }); setIsFormOpen(true); }} className="text-xs text-cyan-200 hover:text-cyan-100"><Pencil size={14} /></button><button onClick={() => { if (window.confirm("حذف وسيلة التواصل؟")) remove.mutate({ id: link.id }); }} className="text-xs text-rose-300 hover:text-rose-200"><Trash2 size={14} /></button></div></article>)}{!links?.length && <p className="py-10 text-center text-sm text-slate-500">لا توجد وسائل تواصل بعد.</p>}</div></section>;
}
