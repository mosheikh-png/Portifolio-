...
function localStorageDir() {
  return path.resolve(process.cwd(), process.env.LOCAL_STORAGE_DIR || "assets/public");
}
...
export async function storagePut(
  ...
  if (isLocalStorage()) {
    ...
    if (!filePath.startsWith(`${baseDir}${path.sep}`)) throw new Error("Storage path escapes local storage directory");
...
export async function storageGetSignedUrl(relKey: string): Promise<string> {
  if (isLocalStorage()) return `/manus-storage/${normalizeKey(relKey)}`;
  const { forgeUrl, forgeKey } = getForgeConfig();
  const key = normalizeKey(relKey);
